...
//creating a list of PS4 games

var UbiGames = from x in videoGames
                    where x.Publisher == "Ubisoft"
                    select x;

//copied from code snippet on https://stackoverflow.com/questions/4003835/sort-list-in-c-sharp-with-linq

UbiGames = UbiGames.OrderBy(x => x.Name).ToList();
...
Console.WriteLine($"Out of {videoGames.Count()} games, {UbiGames.Count()} are from Ubisoft. " +
    $"This means {String.Format("{0:0%}", Convert.ToString(Math.Round(Convert.ToDouble(UbiGames.Count()) / Convert.ToDouble(videoGames.Count()) * 100, 2)))}% of games are from Ubisoft.");

Console.WriteLine("Press enter to continue.");
Console.ReadLine();

//selecting sport genre games from the list of games
